...
float set_speed(float motorL, float motorR)
{
  ...
  if (motorL > 0)
  {
    ...
    digitalWrite(LEFT_BW, LOW);
    analogWrite(LEFT_ACC, PWM_MAX/2 + int(motorL*PWM_MAX)/2);
  }
  ...
  else if (motorL < 0)
  {
    ...
    analogWrite(LEFT_ACC, PWM_MAX/2 + abs(int(motorL*PWM_MAX))/2);
  }
  /* In any other case */
  else
  {
  ...
  if (motorR > 0)
  {
    ...
    digitalWrite(RIGHT_BW, LOW);
    analogWrite(RIGHT_ACC, PWM_MAX/2 + int(motorR*PWM_MAX)/2);
  }
  ...
  else if (motorR < 0)
  {
    ...
    analogWrite(RIGHT_ACC, PWM_MAX/2 + abs(int(motorR*PWM_MAX))/2);
  }
  /* In any other case */
  else
  {
